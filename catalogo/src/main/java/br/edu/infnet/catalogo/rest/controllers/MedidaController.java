package br.edu.infnet.catalogo.rest.controllers;

import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.HttpStatus;
import org.springframework.web.bind.annotation.DeleteMapping;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.PutMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseStatus;
import org.springframework.web.bind.annotation.RestController;

import br.edu.infnet.catalogo.domain.entities.Medida;
import br.edu.infnet.catalogo.domain.services.MedidaService;

@RestController
@RequestMapping("/medidas")
public class MedidaController {

	@Autowired
	MedidaService medidaService;

	@GetMapping
	public List<Medida> listarMedidas() {
		return medidaService.obterTodos();
	}

	@GetMapping("/{id}")
	public Medida obterMedida(@PathVariable Long id) {
		return medidaService.obterPorId(id);
	}

	@PostMapping
	@ResponseStatus(HttpStatus.CREATED)
	public Medida salvarMedida(@RequestBody Medida medida) {
		return medidaService.salvar(medida);
	}

	@PutMapping("/{id}")
	@ResponseStatus(HttpStatus.OK)
	public Medida atualizarMedida(@PathVariable Long id, @RequestBody Medida medida) {
		return medidaService.atualizar(id, medida);
	}

	@DeleteMapping("/{id}")
	@ResponseStatus(HttpStatus.OK)
	public void excluirMedida(@PathVariable Long id) {
		medidaService.excluir(id);
	}
}
